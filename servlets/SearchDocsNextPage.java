...
/**
  Display next page in search result.
  */
public class SearchDocsNextPage extends HttpServlet {
	...
	*/
	public void doGet( HttpServletRequest req, HttpServletResponse res ) throws ServletException, IOException {
		...
		if( next_start > docs.size( ) ) {
			last_page = true ;
			next_start = docs.size() - (docs.size() - start) + 3 ;
		} else
			last_page = false ;
		...
		if (!last_page)
		 htmlStr += "<TR><TD><H4>(" + (next_start / 3 - 9) + "-" + next_start / 3  + ") " + no_of_docs / 3 + "</H4></TD><TD><H4>dokument hittades.</H4></TD></TR>" ;
		   ...
		   htmlStr += "<TR><TD><H4>(" + (next_start / 3 ) + "-" + docs.size() / 3  + ") " + no_of_docs / 3 + "</H4></TD><TD><H4>dokument hittades.</H4></TD></TR>" ;
