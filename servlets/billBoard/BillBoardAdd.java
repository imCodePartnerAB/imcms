...
public class BillBoardAdd extends BillBoard	{
	...
	if ( userHasRightToEdit( imcref, metaId, user ) )
		...
		if(addType.equalsIgnoreCase("DISCUSSION") && ( req.getParameter("ADD") != null || req.getParameter("ADD.x") != null ) )
			...
			billref.sqlUpdateProcedure(sqlQuest) ;
			...
			if (session != null)
			    {
				String latestDiscId = billref.sqlProcedureStr("B_GetLastDiscussionId " +
									      params.getProperty("META_ID") + ", " + aSectionId) ;
				session.setAttribute("BillBoard.disc_id", latestDiscId) ;//Conference.disc_id
			    }

			// Lets redirect to the servlet which holds in us.
			res.sendRedirect("BillBoardDiscView") ;//ConfDiscView
			return ;
		    }
		...
		if(addType.equalsIgnoreCase("REPLY") && ( req.getParameter("ADD") != null || req.getParameter("ADD.x") != null ) )
			...
			String toEmail = billref.sqlProcedureStr("B_GetEmail " + discId);
			if (toEmail== null)
			    {
			...
			String subjectStr = billref.sqlProcedureStr(sqlQuest);
			try {
    ...
    */
	...
	if ( userHasRightToEdit( imcref, metaId, user ) )
		...
		String currSection = "" + billref.sqlProcedureStr("B_GetSectionName " + params.getProperty("SECTION_ID")) ;//GetForumName, FORUM_ID
