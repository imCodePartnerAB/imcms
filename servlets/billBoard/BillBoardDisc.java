...
public class BillBoardDisc extends BillBoard {//ConfDisc
	...
	throws ServletException, IOException
	{
		...
		if (req.getParameter("UPDATE") != null)
		{
			...
			res.sendRedirect(where + "BillBoardDiscView") ;
			return ;
		}
	...
	*/
	...
	throws ServletException, IOException
	{
		...
		{	
			...
			sqlStoredProcOld = "B_GetAllBillsToShow " + aMetaId + ", " + first ;
			String sqlAnswer2[] = rmi.execSqlProcedureExt(confPoolServer, sqlStoredProcOld ) ;
			sqlAnswer=sqlAnswer2;
		...
		File htmlCodeFile)  throws ServletException, IOException
	{
		...
		try
		{
			...
			for(int i = nbrOfCols+1; i<DBArr.length; i += nbrOfCols)
			{
				...
				dataV.add( MetaInfo.getServletPath(req) + "BillBoardReply?") ;
	...
	*/
	...
	/*	String metaId = params.getProperty("META_ID") ;
		...
		sqlQ += "AND forum.forum_id = " + aSectionId + "\n" ;
