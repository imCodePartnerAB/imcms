	...
	**/
	public void doGet(HttpServletRequest req, HttpServletResponse res)
		...
		if ( req.getParameter("ADD_NEW_ROLE") != null || req.getParameter("RENAME_ROLE") != null 
			...
			String[] rolesArr = rmi.execSqlProcedure(server, "RoleAdminGetAll") ;
	...
	**/	
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		if( req.getParameter("VIEW_ADMIN_ROLE_BELONGINGS") != null) {
			res.sendRedirect(MetaInfo.getServletPath(req) + "AdminRoleBelongings") ;
			return ;
		}
		...
		if( req.getParameter("VIEW_ADD_NEW_ROLE") != null) {
			...
			String languagePrefix = rmi.execSqlProcedureStr(server, sqlQ );
	    	
			sqlQ = "RoleGetPermissionsByLanguage " + "'" + languagePrefix + "'";
	    	String[][] rolePermissions = rmi.execProcedureMulti(server, sqlQ);
		...
		if( req.getParameter("VIEW_RENAME_ROLE") != null) {
			...
			if (roleId == null) {
				...
				String msg = "Du måste ange vilken roll som skall ändra på" + "<BR>";
			...
			String currRoleName = rmi.execSqlProcedureStr(server, sqlQ) ;
		...
		if( req.getParameter("VIEW_EDIT_ROLE") != null) {
			...
			String roleId = req.getParameter("ROLE_ID") ;
	  		if (roleId == null) {
	    		String header = "Roles error" ;
	  	  		String msg = "Du måste ange vilken roll som skall redigeras" + "<BR>";
		 	  	this.log("Error in checking roles") ;
	  	  		AdminError err = new AdminError(req,res,header, msg) ;
		 	  	return ;
	    	}
	    	...
	    	String[][] permissionList = rmi.execProcedureMulti(server, sqlQ);
			...
			/* create output page */			
			VariableManager vm = new VariableManager();
			vm.addProperty( "CURRENT_ROLE_NAME", rmi.execSqlProcedureStr( server, "RoleGetName " + roleId ) );
			vm.addProperty( "CURRENT_ROLE_ID", roleId );
	    	vm.addProperty( "ROLE_PERMISSIONS", permissionComponent );
		...
		if( req.getParameter("ADD_NEW_ROLE") != null) {
			...
			if( super.checkParameters(params) == false) {
				String header = "Roles error" ;
				String msg = "Du måste ange ett nytt rollnamn" + "<BR>";
				this.log("Error in checking roles") ;
				AdminError err = new AdminError(req,res,header, msg) ;
				return ;
			}
			...
			RmiLayer rmi = new RmiLayer(user) ;
			String foundRoleName = rmi.execSqlProcedureStr(server, "RoleFindName '" + params.get("ROLE_NAME") + "'") ;
			...
			log( "Sql" + sqlU );
			rmi.execSqlUpdateProcedure(server, sqlU );
			this.doGet(req, res) ;
		...
		if( req.getParameter("VIEW_DELETE_ROLE") != null) {
			...
			if( super.checkParameters(params) == false) {
				String header = "Roles error" ;
				String msg = "Du måste ange ett roll_id" + "<BR>";
	  			this.log("Error in checking roles") ;
				AdminError err = new AdminError(req,res,header, msg) ;
	 	  		return ;
			}
			...
			RmiLayer rmi = new RmiLayer(user) ;
			String affectedMetaIds[] = rmi.execSqlProcedure(server, "RoleDeleteViewAffectedMetaIds " + params.get("ROLE_ID")) ;
	
			// Lets get nbr of affected  metaid:s
			String roleCount = rmi.execSqlProcedureStr(server, "RoleCount " + params.get("ROLE_ID")) ;

			// Lets get the top 50 users:s which will be affected if we delete the role
			String affectedUsers[] = rmi.execSqlProcedure(server, "RoleDeleteViewAffectedUsers " + params.get("ROLE_ID")) ;
			...
			if (affectedUsers.length != 0 || affectedMetaIds.length != 0 ) {
				...
				Html ht = new Html() ;	
				String opt = ht.createHtmlCode("ID_OPTION", "", super.convert2Vector(affectedMetaIds) ) ;
				String users = ht.createHtmlCode("ID_OPTION", "", super.convert2Vector(affectedUsers) ) ;
				VariableManager vm = new VariableManager() ;
