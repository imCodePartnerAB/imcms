...
public class ChatCreator extends ChatBase {
    ...
    private void createChat( int metaId, Chat myChat, IMCPoolInterface chatref, IMCServiceInterface imcref, User user, HttpServletResponse res ) throws IOException {
        ...
        chatref.sqlUpdateProcedure( "C_DeleteChatRooms", new String[]{"" + metaId} );
        ...
        for ( int i = 0; i < vect.size(); i += 2 ) {
            ...
            chatref.sqlUpdateProcedure( "C_AddNewChatRoom", new String[]{"" + metaId, "" + vect.get( i ), "" + vect.get( i + 1 )} );
        }
        ...
        res.sendRedirect( "ChatLogin?login_type=login" );
    ...
    public Vector createTaggs( HttpServletRequest req, Chat chat ) throws ServletException, IOException {
        ...
        taggs.add( "#roomList#" );
        taggs.add( createOptionCode( "", chat.getAllChatGroupsIdAndNameV() ) );
        taggs.add( "#chatRoom#" );
        taggs.add( "" );
        taggs.add( "#msgType#" );
        taggs.add( "" );
