...
public class ChatAdmin extends Administrator{
	...
	if ( noErrors ) {
	    ...
	    String htmlChatElement = imcref.parseDoc( null, this.TEMPLATE_CONF_ELEMENT, languagePrefix );
	    ...
	    String[][] listOfChats = imcref.sqlQueryMulti( "ListChats" );
		...
		String[][] queryResultForum = ChatManager.getStatistics( chatref, sprocetForum );
		...
		for ( int j = 0 ; j < queryResultForum.length ; j++ ) {
		    ...
		    String[][] queryResultDebate = ChatManager.getStatistics( chatref, sprocetDebate );
		...
		if ( queryResultForum.length > 0 ) {
		    ChaterenceTags.put( "SERVLET_URL", "");
