...
*/
	...
	/*	// ************* VERIFY LOGIN TO CONFERENCE **************
		...
		if(loginType.equalsIgnoreCase("login") // && req.getParameter("submit") != null ) {
			...
			if( foundUserInConf == null && okToLogIn == false) {
				...
				if( firstName == null || lastName == null ) {
					String header = "ConfLogin servlet." ;
					ChatError err = new ChatError(req,res,header,62, LOGIN_ERROR_HTML ) ;
					log(header + err.getErrorMsg() + "\n the user exists, but is not a member in this conference") ;
					return ;
				}
		...
		if(loginType.equalsIgnoreCase("ADD_USER") ) {
			...
			if( this.checkUserParameters(userParams) == false) {
				String header = "ConfLogin servlet." ;
				ChatError err = new ChatError(req,res,header,51, LOGIN_ERROR_HTML ) ;
				log(header + err.getErrorMsg()) ;
				return ;
			}
			...
			if(newUserId == null) {
				String header = "ConfLogin servlet." ;
				ChatError err = new ChatError(req,res,header,61, LOGIN_ERROR_HTML ) ;
				return ;
			}
			...
			String header = "ConfLogin servlet." ;
			ChatError err = new ChatError(req,res,header,55, ADD_USER_OK_HTML ) ;
			log(header + err.getErrorMsg()) ;
			return ;
		}
	...
	*/
	...
	if( ! pwd1.equals(pwd2) ) {
	ChatError err = new ChatError(req,res,header,52, LOGIN_ERROR_HTML ) ;
	 			log(header + err.getErrorMsg()) ;
	  	return false ;
	}

	  if( pwd1.length() < 4) {
	ChatError err = new ChatError(req,res,header,53, LOGIN_ERROR_HTML ) ;
	 			log(header + err.getErrorMsg()) ;
	  	return false ;
	 		}
	...
	*/
	...
	throws ServletException, IOException {
		...
		if (userId == null ) {
			String header = "ConfLogin servlet." ;
			ChatError err = new ChatError(req,res,header,59, LOGIN_ERROR_HTML ) ;
