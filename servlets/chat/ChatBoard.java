	...
	public void doGet(HttpServletRequest req, HttpServletResponse res)throws ServletException, IOException{
		...
		if (true){ 
			...
			while(msgIter.hasNext()){
				...
				if (tempMsg.getMsgType() == 101){
					if (privateMsg){ //show private messages
						if (tempMsg.getReciever() == myMember.getUserId()){ //ok it's to mee	
							...
							sendMsgString.append(IMCServiceRMI.parseExternalDoc(imcServer,vLine, "private_to_msg.html","se", "103", libName)+"<br>\n");
							parse = true;
							parsed = true;	
							...
							vLine.add("#reciever#");	vLine.add(tempMsg.getRecieverStr());
							vLine.add("#message#");		vLine.add(tempMsg.getMessage() );
							sendMsgString.append(IMCServiceRMI.parseExternalDoc(imcServer,vLine, "private_from_msg.html","se", "103", libName)+"<br>\n");
							parse = true;						
							parsed = true;
						}
					}//end privateMsg
				...
				if (parse){
					if ( !parsed ) {
						sendMsgString.append(IMCServiceRMI.parseExternalDoc(imcServer,vLine, "standard_msg.html","se", "103", libName)+"<br>\n");
					}
