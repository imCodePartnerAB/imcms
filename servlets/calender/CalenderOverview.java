	...
	*/
	public String makeOverview(HttpServletRequest req, HttpServletResponse res, 
		...
		if (session.getAttribute("calender_view").equals("app") ||
			...
			String[] app = dbmanager.getAppointment(prop, appId);
			String[] player = dbmanager.getPlayerInfo(prop, player_id);
		...
		// Check if delete link was pressed
		else if ( session.getAttribute("calender_view").equals("del") ){
		...
		// Check if user should view the new appointment window
		else if (session.getAttribute("calender_view").equals("NewApp")){
		...
		// Check if any button in the admin panel was pressed
		else if ( session.getAttribute("calender_view").equals("admin") ){
		
		
			if ( req.getParameter("addPlayer") != null ){
			
				String[] calUsers = dbmanager.getCalenderUsers(prop);
				String[] result = dbmanager.getAllUsers(prop);
		...
		// Check if button for choosing users was pushed
		else if ( req.getParameter("usersubmit") != null ){
			...
			if ( action.equalsIgnoreCase("add") ){
				...
				session.setAttribute("calender_player", null);
		...
		// Check if edit title button was pressed
			...
			if (view.equalsIgnoreCase("month")){
				...
				String[] apps = dbmanager.getMyAppointments(prop, meta_id,
			...
			String info[] = dbmanager.getPlayerInfo(prop, player_id);
			if (info.length > 1){
				var.add("#PLAYER#");
			...
			String meta_id = prop.getProperty("calender_meta_id");
			String[] dayApps = dbmanager.getMyAppointments(prop, meta_id, "" + player_id, session);
