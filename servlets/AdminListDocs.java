...
public class AdminListDocs extends Administrator {
    ...
    private String getSelectedDocTypeNames (IMCServiceInterface imcref, String[] selectedDocTypes, String langPrefix){
        String selectedDocTypesName = "" ;
        String[] allDocTypes = imcref.getDocumentTypesInList(langPrefix);

        for (int i=0; i<selectedDocTypes.length; i++){

            for (int j=0; j < (allDocTypes.length)-1; j+=2){
               if( allDocTypes[j].equals(selectedDocTypes[i])){
                   selectedDocTypesName = selectedDocTypesName.toString() + allDocTypes[j+1] + ", ";
                   break;
               }else if (selectedDocTypes[i].equals("0")){
                   selectedDocTypesName = selectedDocTypesName + allDocTypes[j+1] + ", ";
               }
            }
        }

        return StringUtils.stripEnd(selectedDocTypesName, ", ");
    }
