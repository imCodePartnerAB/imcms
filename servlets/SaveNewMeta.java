...
public class SaveNewMeta extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        */
        ...
        String[] user_dt = imcref.sqlProcedure("GetDocTypesForUser", new String[] { parent_meta_id, ""+user.getUserId(), lang_prefix });
        ...
        metaprops.setProperty("date_modified", dateTimeFormatWithSeconds.format(dt));
        metaprops.setProperty("date_created", dateTimeFormatWithSeconds.format(dt));
        ...
        if (req.getParameter("cancel") != null) {
            ...
            String sqlStr = "insert into meta (" + StringUtils.join(sqlInsertColumnNames.iterator(), ',') + ")\n" +
                    ...
                    "SELECT @@IDENTITY";
            String meta_id = imcref.sqlQueryStr(sqlStr, (String[]) sqlInsertValues.toArray(new String[sqlInsertValues.size()]));

            // Save the classifications to the db
            if (classification != null) {
                imcref.sqlUpdateProcedure("Classification_Fix", new String[] {meta_id, classification});
            }

            imcref.sqlUpdateProcedure("InheritPermissions", new String[] { meta_id, parent_meta_id, doc_type});

            // Lets add the sortorder to the parents childlist
            sqlStr = "declare @new_sort int\n" +
                    "select @new_sort = max(manual_sort_order)+10 from childs where meta_id = ? and menu_sort = ?\n" +
                    "if @new_sort is null begin set @new_sort = 500 end\n" +
                    "insert into childs (meta_id, to_meta_id, menu_sort, manual_sort_order, tree_sort_index ) values (?,?,?,@new_sort,'')\n";
            imcref.sqlUpdateQuery(sqlStr, new String[]{parent_meta_id, doc_menu_no, parent_meta_id, meta_id, doc_menu_no});
            log(meta_id);

            // Lets update the parents created_date
            sqlStr = "update meta\n";
            sqlStr += "set date_modified = ?\n";
            sqlStr += "where meta_id = ?";
            imcref.sqlUpdateQuery(sqlStr, new String[] {metaprops.getProperty("date_modified"), parent_meta_id});
            ...
            SaveMeta.setSectionInDbFromRequest(req, imcref, meta_id);
            ...
            if (doc_type.equals("6")) {
                sqlStr = "insert into browser_docs (meta_id, to_meta_id, browser_id) values (?,?,0)";
                imcref.sqlUpdateQuery(sqlStr, new String[] {meta_id,parent_meta_id});
                ...
                String temp[] = imcref.sqlQuery(sqlStr, new String[] {lang_prefix});
                Vector vec = new Vector();
                String temps = null;
                ...
                sqlStr = "select template_id, sort_order,group_id,default_template_1,default_template_2 from text_docs where meta_id = ?";
                String temp[] = imcref.sqlQuery(sqlStr, new String[] {parent_meta_id});
                ...
                imcref.sqlUpdateQuery(sqlStr, new String[] { meta_id, temp[0], temp[1], temp[2], temp[3], temp[4] });
                ...
                if (copyMetaFlag.equals("1") && doc_type.equals("2")) {
                    ...
                    sqlStr = "insert into texts (meta_id,name,text,type) values (?, 1, ?, 1)";
                    imcref.sqlUpdateQuery(sqlStr, new String[] {meta_id,mHeadline});
                    sqlStr = "insert into texts (meta_id,name,text,type) values (?, 2, ?, 1)";
                    imcref.sqlUpdateQuery(sqlStr, new String[] {meta_id,mText});
                }

                // Lets activate the textfield
                sqlStr = "update meta set activate = 1 where meta_id = ?";
