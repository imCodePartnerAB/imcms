...
public class SaveMeta extends HttpServlet {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        if (!imcref.checkDocAdminRightsAny(meta_id_int, user, IMCConstants.PERM_EDIT_HEADLINE | IMCConstants.PERM_EDIT_DOCINFO | IMCConstants.PERM_EDIT_PERMISSIONS)) {	// Checking to see if user may edit this
        ...
        for (int i = 0; i < role_permissions.length; ++i) {
            ...
            if ((
              ...
              // May the user edit the permissions for this particular role?
                ...
                temp_permission_settings.setProperty(String.valueOf(role_id),
                                                     String.valueOf(new_set_id));
            } else {
                mainLog.info("User " + user.getUserId() + " with set_id " + user_set_id
                             + " and permission_set "
                             + user_perm_set
        ...
        */
        ...
        if (req.getParameter("define_set_1") != null) {	// If user want's to edit permission-set 1
            user.put("temp_perm_settings",
                     new Object[]{
                         String.valueOf(meta_id), metaprops, temp_permission_settings,
            ...
            out.write(
                    MetaDataParser.parseMetaPermission(meta_id, meta_id, user, host, "change_meta_rights.html"));
            return;
        }
        ...
        if (sqlUpdateColumns.size() > 0) {
            ...
            updateParameters[updateParameters.length - 1] = meta_id;
        ...
        for (int i = 0; i < role_permissions.length; ++i) {
            ...
            imcref.sqlUpdateProcedure("SetRoleDocPermissionSetId", new String[]{role_id, meta_id, new_set_id});
        }
        ...
        if (created_datetime != null) {
            imcref.sqlUpdateQuery("update meta set date_created = ? where meta_id = ?", new String[] {created_datetime, meta_id});
        }
        if (modified_datetime != null) {
            imcref.sqlUpdateQuery("update meta set date_modified = ? where meta_id = ?", new String[] {modified_datetime, meta_id});
        }

        // Update the date_modified for all parents.
        imcref.sqlUpdateProcedure("UpdateParentsDateModified", new String[] { meta_id });

        if (imcref.checkAdminRights(user) || imcref.checkDocAdminRights(meta_id_int,user,IMCConstants.PERM_EDIT_DOCINFO)) {
            setSectionInDbFromRequest(req, imcref, meta_id);
        }
    ...
    static void setSectionInDbFromRequest(HttpServletRequest req, IMCServiceInterface imcref,
        ...
        if (null != sectionIdStrings) {
            imcref.sqlUpdateQuery( "DELETE FROM meta_section WHERE meta_id = ?",
                                   new String[]{"" + metaId} );
            for (int i = 0; i < sectionIdStrings.length; i++) {
                try {
                    int sectionId = Integer.parseInt(sectionIdStrings[i]);
                    imcref.sqlUpdateQuery("INSERT INTO meta_section VALUES(?,?)",
                                          new String[]{"" + metaId, "" + sectionId});
