...
*/
	...
	// Lets get the html code for the adminbuttons, if it should be shown for the user
      //log("before adminBUttonlink: " + imagePath) ;
		String adminbtn = this.getAdminButtonLink(req, user) ;
      // log("before UNadminBUttonlink: " + imagePath) ;
		String unAdminBtn = this.getUnAdminButtonLink(req, user) ;
...
public String getAdminButtonLink(HttpServletRequest req,imcode.server.User user )
 throws ServletException, IOException {
  ...
  String ConfPoolServer = Utility.getDomainPref("conference_server",host) ;
	...
	if(this.getAdminRights(imcServer, metaId, user) == false) return adminBtn ;
   log("After getAdminRights") ;
	adminBtn += "<img src=\"" + this.getExternalImageFolder(req) ;
	adminBtn += "admin.jpg"  + "\" border=0>";
	return adminBtn ;
} // End CreateAdminHtml
...
/**
	...
	to the adminservlet.
*/
public String getUnAdminButtonLink(HttpServletRequest req,imcode.server.User user )
 throws ServletException, IOException {
 ...
 // Lets get serverinformation
	...
	if(this.getAdminRights(imcServer, metaId, user) == false) return adminBtn ;
	adminBtn += "<img src=\"" + this.getExternalImageFolder(req) ;
	adminBtn += "avsl_adm.jpg"  + "\" border=0>";
	return adminBtn ;
} // End CreateAdminHtml
