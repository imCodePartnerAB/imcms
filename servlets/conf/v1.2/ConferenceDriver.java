...
public class ConferenceDriver{
	...
	ConferenceDriver dd = new ConferenceDriver() ;
   String sql = "AdminStatistics1 4589, '1991-01-01 00:00', '2070-01-01 00:00', 1" ;
   String server = "rmi://127.0.0.1:1099/IMCPool0" ;

   try {
    String[][] arr = ConfManager.getStatistics(server, sql) ;
    dd.log("Längd" + arr.length) ;
   } catch(Exception e) {
     dd.log(e.getMessage() ) ;
   ...
   //StringBuffer strBuff = new StringBuffer("Rad1 \n Rad2") ;

   //dd.log(strBuff.toString()) ;
   //dd.log("Htmlkoll: " + Html.replace(strBuff, '\n', "<BR>")) ;
   /*
   ...
   for( int i = 0 ; i < strBuff.length(); i++ ) {
      ...
      if('\n' == aChar) {
         dd.log("Träff: " + i) ;
         strBuff = strBuff.replace(i,i,"<BR>") ;
         i+=4 ;
      	// strBuff = strBuff.insert(i,"<BR>") ;
      }
   }
   dd.log(retBuff.toString()) ;
 */
//  String str = RmiConf.getLoginUrl() ;
//	dd.log("RMICONF: " + str) ;
...
/*
	ConferenceDriver dd = new ConferenceDriver() ;
	String path = "c:/imcserver/imcode/external/diverse/" ;
  String prefsFile = "conference.cfg" ;
...
//  String prefsFile = "conference.cfg" ;
	...
	mySetAcc.setDelimiter("=") ;
	dd.log("delimiter: " + mySetAcc.getDelimiter()) ;
	mySetAcc.loadSettings() ;
	dd.log("start_url: " + mySetAcc.getSetting("start_url")) ;
	dd.log("imcserv_ip: " + mySetAcc.getSetting("imcserv_ip")) ;
