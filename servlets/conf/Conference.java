...
public class Conference extends HttpServlet {
    ...
    public MetaInfo.Parameters getConferenceSessionParameters( HttpServletRequest req )
            throws ServletException, IOException {
     ...
     **/
    protected String getTemplateLibName( IMCPoolInterface confref, String meta_id )
            throws ServletException, IOException {
        String sqlQ = "A_GetTemplateLib " + meta_id;
        String libName = confref.sqlProcedureStr( sqlQ );
        if ( libName == null ) {
            libName = "original";
        }
        libName += "/";
    ...
    public boolean prepareUserForConf( HttpServletRequest req, HttpServletResponse res,
        ...
        String lastLoginDate = confref.sqlProcedureStr( sqlStoredProc );
        String firstName = "";
        String lastName = "";
        ...
        if ( lastLoginDate == null ) {
            ...
            confref.sqlUpdateQuery( firstTimeInConfSql );

            // Ok, try to get the lastLoginDate now and validate it
            lastLoginDate = confref.sqlProcedureStr( sqlStoredProc );
            ...
            firstName = confref.sqlProcedureStr( sqlName );
            sqlName = "A_GetConfLoginNames " + metaId + ", " + loginUserId + ", " + 2;
            lastName = confref.sqlProcedureStr( sqlName );
        } // end else
        ...
        confref.sqlUpdateQuery( sqlQuest );
        ...
        if ( session != null ) {
            ...
            String aForumId = confref.sqlProcedureStr( "A_GetFirstForum " + params.getMetaId());
            session.setAttribute( "Conference.forum_id", aForumId );

            // Ok, Lets get the last discussion in that forum
            String aDiscId = confref.sqlProcedureStr( "A_GetLastDiscussionId " +
                                                      params.getMetaId() + ", " + aForumId );
    ...
    protected void setSessionAttributes( HttpSession session, Properties params ) {
        ...
        setSessionAttributes(session, new MetaInfo.Parameters(metaId,parentMetaId));
    }

    protected void setSessionAttributes( HttpSession session, MetaInfo.Parameters params) {
