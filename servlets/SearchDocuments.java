...
public class SearchDocuments extends HttpServlet {
   ...
   // Lets run the question
	//	if( req.getParameter("search") != null || req.getParameter("search_prev") != null ) {
			//Vector docs = IMCServiceRMI.searchDocs( imcserver,meta_id,user,question_field,search_type,search_prep,
			//  string_match,search_area ) ;
      //log("QUIZ=" + question_field + " SEARCH_TYPE=" + search_type + " SEARCH_PREP=" + search_prep +
      //  " SEARCH_MATCH=" + string_match + " SEARCH_AREA=" + search_area) ;

      String sqlStr = buildSqlStr(question_field, search_type,
        search_prep, string_match, search_area) ;
   ...
   // Lets calculate our min value in the array
    ...
    // Lets fix the previous button. Lets first recalculate our current startvalue
        if(start > (nbrToShow)) {
         ...
         // log("Start är större än " + nbrToShow) ;
  ...
  //    this.doGet(req,res) ;
   ...
   // Lets get the html file we use as template
       //log("User: " + user.getString("lang_id")) ;
       String langPrefix = "SE" ; // IMCServiceRMI.getLanguage(imcserver, user) ;
       //String langPrefix = "SE" ; //
       Properties props = this.getSummaryProps(servlet_url) ;
       //htmlStr = IMCServiceRMI.parseDoc(imcserver,outV,"search_doc.html",langPrefix);
      ...
      // Lets NOT search on codes
       ...
       // THE NEW CLASSIFICATION STYLE
        ...
        sqlStr += "AND (" ;
          ...
          //sqlStr += "( class.code LIKE '" + match + sWord + match + "' OR class.code LIKE '" + match + sWord + match + "' )\n" ;
          sqlStr += "( class.code LIKE '" + match + sWord + match + "' )\n" ;
				  // sqlStr += " classification LIKE '%" + tokens.elementAt(i).toString() + match + "'" ;
...
/*		try {
	  		...
	  		log("Should have seen four parameter names");
			throw new UnavailableException (this, "Invalid parameter.");
		}
*/
	}
  ...
  */
  public void log(String str) {
    super.log(str) ;
    //System.out.println("SearchDocs: " + str) ;
  }
