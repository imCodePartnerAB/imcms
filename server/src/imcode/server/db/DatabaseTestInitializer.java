...
public class DatabaseTestInitializer {
    ...
    static DatabaseService static_initMimer() throws IOException {
        DatabaseService dbService = new MimerDatabaseService("localhost", MIMER_PORT, "imcmstest", "sysadm", "trexus", FILE_PATH );
    ...
    static DatabaseService static_initSqlServer() throws IOException {
        DatabaseService dbService = new SQLServerDatabaseService( "ratatosk", SQLSERVER_PORT, "kreiger_imcmstest", "sa", "nonac", FILE_PATH );
        dbService.setupDatabaseWithTablesAndData();
        dbService.createTestData();
        return dbService;
    }
    ...
    static DatabaseService static_initMySql() throws IOException {
        DatabaseService dbService = new MySQLDatabaseServer( "localhost", MYSQL_PORT, "imcmstest", "root", "", FILE_PATH );
        dbService.setupDatabaseWithTablesAndData();
        dbService.createTestData();
        return dbService;
    }
