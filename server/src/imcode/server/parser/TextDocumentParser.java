...
public class TextDocumentParser implements imcode.server.IMCConstants {
    ...
    static {
        ...
        try {
            ...
            IMCMS_TAG_PATTERN = patComp.compile( "<\\?imcms:([-\\w]+)(.*?)\\?>",
                    Perl5Compiler.SINGLELINE_MASK
                    | Perl5Compiler.READ_ONLY_MASK );
            HASHTAG_PATTERN = patComp.compile( "#[^ #\"<>&;\\t\\r\\n]+#",
                    Perl5Compiler.READ_ONLY_MASK );
            MENU_PATTERN = patComp.compile( "<\\?imcms:menu(.*?)\\?>(.*?)<\\?\\/imcms:menu\\?>",
                    Perl5Compiler.SINGLELINE_MASK
                    | Perl5Compiler.READ_ONLY_MASK );

            READRUNNER_END_TITLE_PATTERN = patComp.compile( "(</title>)",
                    Perl5Compiler.READ_ONLY_MASK
                    | Perl5Compiler.CASE_INSENSITIVE_MASK );
            READRUNNER_END_HEAD_PATTERN = patComp.compile( "(</head>)",
                    Perl5Compiler.READ_ONLY_MASK
                    | Perl5Compiler.CASE_INSENSITIVE_MASK );
            READRUNNER_START_BODY_PATTERN = patComp.compile( "(<body.*?)(>)",
                    Perl5Compiler.READ_ONLY_MASK
                    | Perl5Compiler.CASE_INSENSITIVE_MASK );
            READRUNNER_END_BODY_PATTERN = patComp.compile( "(</body>)",
                    Perl5Compiler.READ_ONLY_MASK
                    | Perl5Compiler.CASE_INSENSITIVE_MASK );
    ...
    */
    ...
    public String parsePage( DocumentRequest documentRequest, int flags,
    ...
    public String parsePage( DocumentRequest documentRequest, int flags, int includelevel,
        ...
        try {
            ...
            String[] user_permission_set = serverObject.sqlProcedure( "GetUserPermissionSet", metaIdUserIdPair );
            ...
            String[] included_docs = serverObject.sqlProcedure( "GetIncludes", new String[]{"" + meta_id} );
                ...
                if (vectT.length > 0) {
                    try {
                        ...
                        if (temp_template > 0) {
                            ...
                            documentRequest.getDocument().setTemplate(
                                    new Template( temp_template, template_name ) );
                        }
            ...
            if (templatemode) {
                templategroups = new ArrayList( Arrays.asList( serverObject.sqlProcedure( "GetTemplategroupsForUser", new String[]{"" + meta_id, "" + user.getUserId()} ) ) );
                ...
                templates = new ArrayList( Arrays.asList( serverObject.sqlProcedure( "GetTemplatesInGroup", new String[]{"" + selected_group} ) ) );

                groupnamevec = Arrays.asList( serverObject.sqlQuery( "select group_name from templategroups where group_id = ?", new String[]{group_id} ) );
            ...
            String emphasize_string = fileCache.getCachedFileString(
                    new File( admintemplate_path, "textdoc/emphasize.html" ) );
            ...
            */
            ...
            tags.setProperty( "#lastDate#", dateTimeFormatWithSeconds.format( myDoc.getModifiedDatetime() ) );
            ...
            tags.setProperty( "#webMasterEmail#",
                    serverObject.getSystemData().getWebMasterAddress() );
            tags.setProperty( "#serverMaster#", serverObject.getSystemData().getServerMaster() );
            tags.setProperty( "#serverMasterEmail#",
                    serverObject.getSystemData().getServerMasterAddress() );
            ...
            tags.setProperty( "#readrunner_quote_substitution_count#",
                    "#readrunner_quote_substitution_count#" );

            // Give the user a row of buttons if he is privileged enough.
            if ((serverObject.checkDocAdminRights( meta_id, user )
                    || serverObject.checkUserAdminrole( user.getUserId(), 2 ))
                    && flags >= 0) {
            ...
            if (templatemode) {	//Templatemode! :)
                ...
                toload.setProperty( "#changePage#",
                        (new File( admintemplate_path, "textdoc/inPage_admin.html" )).getPath() );
            }  // if (templatemode)
            ...
            if (menumode) {
                ...
                final int[] docTypesSortOrder = { IMCConstants.DOCTYPE_TEXT,
                                                  0, // "Befintligt dokument"
                                                  IMCConstants.DOCTYPE_FILE,
                                                  IMCConstants.DOCTYPE_URL,
                                                  IMCConstants.DOCTYPE_HTML,
                                                  IMCConstants.DOCTYPE_BROWSER,
                                                  IMCConstants.DOCTYPE_CONFERENCE,
                                                  IMCConstants.DOCTYPE_CHAT,
                                                  IMCConstants.DOCTYPE_BILLBOARD,
                                                  IMCConstants.DOCTYPE_DIAGRAM,
                                                  IMCConstants.DOCTYPE_CALENDER, };
                ...
                toload.setProperty( "addDoc",
                        (new File( admintemplate_path, "textdoc/add_doc.html" )).getPath() );
                toload.setProperty( "saveSortStart",
                        (new File( admintemplate_path, "textdoc/sort_order.html" )).getPath() );
                toload.setProperty( "saveSortStop",
                        (new File( admintemplate_path, "textdoc/archive_del_button.html" )).getPath() );
                toload.setProperty( "sort_button",
                        (new File( admintemplate_path, "textdoc/sort_button.html" )).getPath() );

                // Some tags to parse in the files we'll load.
                temptags.setProperty( "#doc_types#", doc_types_sb.toString() );	// The doc-types.
                temptags.setProperty( "#docTypesSortOrder" + sort_order + "#", "checked" );	// The sortorder for this document.
            } // if (menumode)
            ...
            imcode.server.parser.MapSubstitution temptagsmapsubstitution = new imcode.server.parser.MapSubstitution(
                    temptags, false );
            ...
            try {
                ...
                while (propenum.hasMoreElements()) {
                    ...
                    String templatebufferstring = fileCache.getCachedFileString(
                            new File( templatebufferfilename ) );
                    // Humm... Now we must replace the tags in the loaded files too.
                    templatebufferstring = org.apache.oro.text.regex.Util.substitute( patMat,
                            HASHTAG_PATTERN,
                            temptagsmapsubstitution,
                            templatebufferstring,
            ...
            StringBuffer templatebuffer = new StringBuffer(
                    fileCache.getCachedFileString(
                            new File( templatePath, "text/" + template_id + ".html" ) ) );
            ...
            MenuParserSubstitution menuparsersubstitution = new imcode.server.parser.MenuParserSubstitution(
                    documentRequest, menus, menumode, tags );
            HashTagSubstitution hashtagsubstitution = new imcode.server.parser.HashTagSubstitution(
                    tags, numberedtags );
            ImcmsTagSubstitution imcmstagsubstitution = new imcode.server.parser.ImcmsTagSubstitution(
                    this,
                    documentRequest,
                    templatePath,
            ...
            while (pit.hasNext()) {
                ...
                // Aah... the magic of OO...
                nextbit = org.apache.oro.text.regex.Util.substitute( patMat, MENU_PATTERN,
                        menuparsersubstitution,
                        nextbit,
                        org.apache.oro.text.regex.Util.SUBSTITUTE_ALL );

                // Parse the <?imcms:tags?>
                nextbit = org.apache.oro.text.regex.Util.substitute( patMat, IMCMS_TAG_PATTERN,
                        imcmstagsubstitution, nextbit,
                        org.apache.oro.text.regex.Util.SUBSTITUTE_ALL );

                // Parse the hashtags
                nextbit = org.apache.oro.text.regex.Util.substitute( patMat, HASHTAG_PATTERN,
                        hashtagsubstitution, nextbit,
                        org.apache.oro.text.regex.Util.SUBSTITUTE_ALL );
            ...
            if (readrunnerQuoteSubstitutionCount > 0) {
                ...
                String readrunner_script_frag = serverObject.parseDoc(
                        readrunnerSubstitutionCountVector,
                ...
                returnresult = org.apache.oro.text.regex.Util.substitute( patMat,
                        READRUNNER_END_TITLE_PATTERN,
                        new Perl5Substitution(
                                readrunner_titlesuffix_frag
                        + "$1" ),
                        returnresult );

                // Insert the script fragment at the end of the head
                returnresult = org.apache.oro.text.regex.Util.substitute( patMat,
                        READRUNNER_END_HEAD_PATTERN,
                        new Perl5Substitution(
                                readrunner_script_frag
                        + "$1" ),
                        returnresult );

                // Insert the body-events and panel in and after the body tag.
                returnresult = org.apache.oro.text.regex.Util.substitute( patMat,
                        READRUNNER_START_BODY_PATTERN,
                        new Perl5Substitution(
                                "$1"
                        + readrunner_bodyevents_frag
                        + "$2"
                        + readrunner_panel_frag ),
                        returnresult );

                // Insert the buffer fragment at the end of the body
                returnresult = org.apache.oro.text.regex.Util.substitute( patMat,
                        READRUNNER_END_BODY_PATTERN,
                        new Perl5Substitution(
                                readrunner_buffer_frag
                        + "$1" ),
                        returnresult );
            ...
            */
            if (emp != null) { // If we have something to emphasize...
                ...
                while (patMat.contains( emp_input, HTML_TAG_PATTERN )) {
                    ...
                    non_html_tag_string = emphasizeString( non_html_tag_string, emp,
                            emphasize_substitution, patMat );
    ...
    private String emphasizeString( String str,
        ...
        for (int i = 0; i < emp.length; ++i) {
            try {
                ...
                log.warn(
                        "Dynamic Pattern-compilation failed in IMCService.emphasizeString(). Suspected bug in jakarta-oro Perl5Compiler.quotemeta(). The String was '"
                        + emp[i]
                        + "'",
                        ex );
            }
