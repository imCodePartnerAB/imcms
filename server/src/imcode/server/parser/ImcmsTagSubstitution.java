        ...
        if ( null != ( attributevalue = attributes.getProperty( "no" ) ) ) {	    // If we have the attribute no="number"...
            ...
            try {
                ...
                if ( urlStr.startsWith( "/" ) ) {  // lets add hostname if we got a relative path
                    urlStr = "http://" + documentRequest.getHttpServletRequest().getServerName() + urlStr;
                }
                ...
                urlConnection.setRequestProperty( "User-Agent", documentRequest.getHttpServletRequest().getHeader("User-agent") );
