...
import imcode.util.ReadrunnerFilter;
...
class ImcmsTagSubstitution implements Substitution, IMCConstants {
    ...
    static {
        ...
        try {
            ...
            IMCMS_TAG_ATTRIBUTES_PATTERN = patComp.compile("\\s*(\\w+)\\s*=\\s*([\"'])(.*?)\\2",
                    Perl5Compiler.SINGLELINE_MASK
                    | Perl5Compiler.READ_ONLY_MASK);
            HTML_PREBODY_PATTERN = patComp.compile("^.*?<[Bb][Oo][Dd][Yy].*?>",
                    Perl5Compiler.SINGLELINE_MASK
                    | Perl5Compiler.READ_ONLY_MASK);
            HTML_POSTBODY_PATTERN = patComp.compile("<\\/[Bb][Oo][Dd][Yy]>.*$",
                    Perl5Compiler.SINGLELINE_MASK
                    | Perl5Compiler.READ_ONLY_MASK);
    ...
    ImcmsTagSubstitution(TextDocumentParser textdocparser, DocumentRequest documentRequest,
                                File templatepath,
                                String[] included_list, boolean includemode, int includelevel, File includepath,
        ...
        for (Iterator i = Arrays.asList(included_list).iterator(); i.hasNext();) {
    ...
    private String tagSections(Properties attributes) {
        String[] section = document.getSections();

        String separator = attributes.getProperty("separator", ",");

        return Utility.joinArray(section, separator);
    }
    ...
    private String tagInclude(Properties attributes, PatternMatcher patMat) {
        ...
        if (null != (attributevalue = attributes.getProperty("no"))) {	    // If we have the attribute no="number"...
            ...
            try {
                if (includeLevel > 0) {
                    ...
                    String documentStr = textDocParser.parsePage(includedDocumentRequest, -1,
                            includeLevel - 1, paramsToParse);
                    documentStr = org.apache.oro.text.regex.Util.substitute(patMat,
                            HTML_PREBODY_PATTERN,
                            NULL_SUBSTITUTION,
                            documentStr);
                    documentStr = org.apache.oro.text.regex.Util.substitute(patMat,
                            HTML_POSTBODY_PATTERN,
                            NULL_SUBSTITUTION,
                            documentStr);
                    return documentStr;
                }
        ...
        try {
            if (includeMode) {
                ...
                String langPrefix = documentRequest.getUser().getLangPrefix();
                return imcode.util.Parser.parseDoc(
                        fileCache.getCachedFileString(
                                new File(templatePath, langPrefix + "/admin/change_include.html")),
                        new String[]{
                ...
                int included_meta_id = Integer.parseInt(
                        (String) included_docs.get(String.valueOf(no)));
    ...
    private String tagText(Properties attributes, PatternMatcher patMat) {
        ...
        if (null != filter && "readrunner".equalsIgnoreCase(filter)) {
            ...
            result = readrunnerFilter.filter(result, patMat,
                    parserParameters.getReadrunnerParameters());
        }
        ...
        if (textMode) {
            ...
            String langPrefix = documentRequest.getUser().getLangPrefix();
            File admin_template_file = new File(templatePath,
                    langPrefix + "/admin/textdoc/admin_text.frag");
            try {
                finalresult = imcode.util.Parser.parseDoc(
                        fileCache.getCachedFileString(admin_template_file), replace_tags);
            } catch (IOException ex) {
     ...
     * @param patMat     A pattern matcher.
     */
    private String tagImage(Properties attributes, PatternMatcher patMat) {
        String mode = attributes.getProperty("mode");
        if ((mode != null && !"".equals(mode))
        ...
        if (imageMode) {
            String[] replace_tags = getLabelTags(attributes, noStr, finalresult);
            String langPrefix = documentRequest.getUser().getLangPrefix();
            File admin_template_file = "".equals(result)
                    ? new File(templatePath,
                            langPrefix + "/admin/textdoc/admin_no_image.frag")
                    : new File(templatePath,
                            langPrefix + "/admin/textdoc/admin_image.frag");
    ...
    private String tagDatetime(Properties attributes) {
        String format = attributes.getProperty("format") == null
                ? DATETIME_FORMAT_STD : attributes.getProperty("format");
        ...
        if (type != null) {
            ...
            if ("now".startsWith(type)) {
                ...
                return "<!-- <?imcms:datetime ... type=\"" + type
    ...
    private String tagUser(Properties attributes) {
        ...
        if (attrib != null && !"".equals(attrib)) {
            ...
            if (attrib.equals("firstname")) {
                ...
                String[][] phoneNbr = serverObject.sqlProcedureMulti(
                        "GetUserPhoneNumbers", new String[]{"" + userId});
                if (phoneNbr != null) {
