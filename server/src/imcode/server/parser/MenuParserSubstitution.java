...
class MenuParserSubstitution implements Substitution {
    ...
    MenuParserSubstitution(DocumentRequest documentRequest, Map menus, boolean menumode,
                                  Properties tags) {
    ...
    private String nodeMenuParser(int menuId, String menutemplate, Properties menuattributes,
     ...
     */
    private void nodeMenu(Element menuNode, StringBuffer result, Menu currentMenu,
                          PatternMatcher patMat) {
        ...
        if (menuNode.getChildElement("menuloop") == null) {
            nodeMenuLoop(new SimpleElement("menuloop", null, menuNode.getChildren()), result,
                    currentMenu, menuAttributes, patMat); // The imcms:menu contained no imcms:menuloop, so let's create one, passing the children from the imcms:menu
        } else {
                ...
                switch (menuNodeChild.getNodeType()) { // Check the type of the child-node.
                    ...
                    case Node.ELEMENT_NODE: // An element-node
                        if ("menuloop".equals(((Element) menuNodeChild).getName())) { // Is it an imcms:menuloop?
                            nodeMenuLoop((Element) menuNodeChild, result, currentMenu,
                                    menuAttributes, patMat);
                        } else {
    ...
    private void parseMenuItem(StringBuffer result, String template, Substitution substitution,
                               PatternMatcher patMat) {
        result.append(
                org.apache.oro.text.regex.Util.substitute(patMat, HASHTAG_PATTERN, substitution,
                        template,
