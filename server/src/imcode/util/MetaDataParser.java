...
public class MetaDataParser {
     ...
     */
    static public String parseMetaData(String meta_id, String parent_meta_id, User user,
        ...
        if (currentuser_set_id == 0 || (currentuser_perms & 2) != 0) {
            return getMetaDataFromDb(meta_id, parent_meta_id, user, host, "adv_change_meta.html",
                                     false);
        } else {
     ...
     * parses the information into the change_meta.html (the plain admin mode file).
     */
    static public String parseMetaPermission(String meta_id, String parent_meta_id, User user,
                                             String host, String htmlFile) throws IOException {
        //	return getMetaDataFromDb(meta_id, parent_meta_id, user, host, "change_meta.html") ;
    ...
    static public String getMetaDataFromDb(String meta_id, String parent_meta_id, User user, String host,
        ...
        if (temp_perm_settings != null && meta_id.equals(temp_perm_settings[0])) {		// Make sure this is the right document.
            ...
            while (temp_enum.hasMoreElements()) {
                ...
                ((String[]) hash.get(temp_key))[0] = (String) ((Hashtable) temp_perm_settings[1]).get(
                        temp_key);
            }
        }
        ...
        for (int i = 0; i < metatable.length; i += 3) {
            ...
            if (NORMAL.equals(metatable[i + 2])) {			// This is not a checkbox or an optionbox
                ...
                if (!temp.equals(metatable[i + 1])) {	// If it is equal to the nullvalue, it must not appear (i.e. equal null)
                    if (htmlStr.indexOf(tag) == -1) {
                        checks += "<input type=\"hidden\" name=\"" + metatable[i] + "\" value=\""
                                  + temp
                                  + "\">";
                    } else {
                        vec.add(tag);
        ...
        sqlStr = "select rtrim(first_name)+' '+rtrim(last_name) from users join meta on users.user_id = meta.owner_id and meta.meta_id = "
                 + meta_id;
    ...
    public static void getSectionDataFromDbAndAddSectionRelatedTagsToParseList(
        ...
        String[][] documentSections = imcref.sqlProcedureMulti("SectionGetInheritId " + meta_id);
        ...
        String[][] allSectionsArray = imcref.sqlProcedureMulti("SectionGetAll");
     ...
     */
    static public void getRolesFromDb(String meta_id, User user, String host, Vector vec)
            throws IOException {
        ...
        StringBuffer roles_rights = new StringBuffer(
                imcref.parseDoc(null, "roles_rights_table_head.html", lang_prefix));
        ...
        for (int i = 0; i < role_permissions.length; ++i) {
            ...
            if (role_set_id == IMCConstants.DOC_PERM_SET_NONE) {
                ...
                roles_rights.append(
                        "<input type=\"hidden\" name=\"role_" + role_id + "\" value=\"4\">");
            ...
            vec2.add("#user_role#");
            vec2.add(
                    String.valueOf(IMCConstants.DOC_PERM_SET_FULL).equals(role_permissions[i][3])
                    ? "" : "*");
            ...
            for (int j = IMCConstants.DOC_PERM_SET_FULL; j <= IMCConstants.DOC_PERM_SET_NONE; ++j) { // From DOC_PERM_SET_FULL to DOC_PERM_SET_NONE (0 to 4)
                ...
                if (user_set_id <= role_set_id		// User has more privileged set_id than role
                    ...
                    && (user_set_id != IMCConstants.DOC_PERM_SET_RESTRICTED_1
                        ...
                        || (currentdoc_perms
                            & IMCConstants.DOC_PERM_RESTRICTED_1_ADMINISTRATES_RESTRICTED_2)
                           != 0))	// User has set_id 1, and may modify set_id 2?
                {
                    vec2.add(
                            "<input type=\"radio\" name=\"role_" + role_id + "\" value=\"" + j + "\" "
        ...
        if (user_set_id < 2) {
            ...
            if (user_set_id == IMCConstants.DOC_PERM_SET_FULL) {
                ...
                String sets_precedence = imcref.parseDoc(perm_vec,
                                                         "permissions/sets_precedence.html",
                                                         lang_prefix);
                ftr.add("#sets_precedence#");
                ftr.add(sets_precedence);
                ftr.add("#set_1#");
                ftr.add(imcref.parseDoc(null, "permissions/set_1_button.html", lang_prefix));
                ftr.add("#set_2#");
                ftr.add(imcref.parseDoc(null, "permissions/set_2_button.html", lang_prefix));
                if (doc_type == IMCConstants.DOCTYPE_TEXT) {
                    ftr.add("#new_set_1#");
                    ftr.add(imcref.parseDoc(null, "permissions/new_set_1_button.html", lang_prefix));
                    ftr.add("#new_set_2#");
                    ...
                    default_templates = getDefaultTemplateOptionList(imcref, user,
                                                                     temp_default_templates,
                                                                     meta_id, lang_prefix, true);
    ...
    private static synchronized String getDefaultTemplateOptionList(IMCServiceInterface imcref,
        ...
        if (def_templates == null) { //if we dont already have the ones to mark as selected
            def_templates = imcref.sqlQuery(
                    "SELECT default_template_1,default_template_2 FROM text_docs WHERE meta_id="
                    + meta_id);
        }
        ...
        if (restr_1) {
            ...
            for (int i = 0; i < templates.length; i++) {
                ...
                tempStr += "<option value=\"" + templates[i][0] + "\"" + selected + ">"
                           + templates[i][1]
                           + "</option>\n\t";
            }
        ...
        for (int i = 0; i < templates.length; i++) {
            ...
            options_templates_2 += "<option value=\"" + templates[i][0] + "\"" + selected + ">"
                                   + templates[i][1]
                                   + "</option>\n\t";
        }
    ...
    public static String parsePermissionSet(int meta_id, User user, String host, int set_id,
        ...
        AdminButtonParser vec = new AdminButtonParser(
                "permissions/define_permission_" + doc_type + "_", ".html", user_set_id, user_perm_set) {
        ...
        String[] permissionset = imcref.sqlProcedure(
                "Get" + newstr + "PermissionSet " + meta_id + "," + set_id + "," + lang_prefix);
        ...
        String[] user_tg = imcref.sqlProcedure(
                "GetTemplateGroupsWith" + newstr + "Permissions " + meta_id + "," + user_set_id);
        ...
        StringBuffer doc_specific = new StringBuffer(
                imcref.parseDoc(null, "permissions/define_permissions_" + doc_type + ".html",
                                lang_prefix));
        ...
        if (for_new) {
            complete = new StringBuffer(
                    imcref.parseDoc(null, "permissions/define_new_permissions.html", lang_prefix));
        } else {
            complete = new StringBuffer(
                    imcref.parseDoc(null, "permissions/define_permissions.html", lang_prefix));
        }
