...
class UserFilterUI extends CustomLayout("admin/access/user/projection/filter") with UndefinedSize {
  val chkText = new CheckBox("users_projection.filter.chk_text".i) with ExposeValueChange[Boolean] with Immediate
  ...
  val chkRoles = new CheckBox("users_projection.filter.chk_roles".i) with ExposeValueChange[Boolean] with Immediate
  val tcsRoles = new TwinColSelect with MultiSelect[RoleId] with TCSDefaultI18n
  val chkShowInactive = new CheckBox("users_projection.filter.chk_show_inactive".i) with ExposeValueChange[Boolean] with Immediate
