...
class SessionCounterManager {
  val view = new SessionCounterManagerView |>> {
    w =>
      ...
      w.miEdit.setCommandHandler {
        _ =>
          new OkCancelDialog("Edit session counter") |>> {
            dlg =>
              dlg.mainComponent = new SessionCounterForm |>> {
                c =>
                  SessionCounter.get |> {
                    sc =>
      ...
      w.miReset.setCommandHandler {
        _ =>
          new ConfirmationDialog("Reset session counter?") |>> {
            dlg =>
              dlg.setOkButtonHandler {
                Current.ui.privileged(permission) {
