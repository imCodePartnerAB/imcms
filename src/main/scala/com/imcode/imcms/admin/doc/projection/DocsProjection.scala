...
class DocsProjection(val user: UserDomainObject, multiSelect: Boolean = true) extends Publisher[Seq[DocumentIdentity]] with ImcmsServicesSupport with Log4jLoggerSupport {
  ...
  private val selectionRef = new AtomicReference(Seq.empty[DocumentIdentity])
  ...
  docsView.addValueChangeHandler { _ =>
    def fieldsToI8nDocRef(fields: DocumentStoredFields): DocumentIdentity = {
      val language = imcmsServices.getDocumentI18nSupport.getByCode(fields.languageCode())
      DocumentIdentity.of(fields.id(), fields.versionNo(), language)
    }
...
//    }
  ...
  def selection: Seq[DocumentIdentity] = selectionRef.get
