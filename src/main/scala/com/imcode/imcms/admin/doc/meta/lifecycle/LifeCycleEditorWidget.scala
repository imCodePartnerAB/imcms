...
class LifeCycleEditorWidget extends VerticalLayout with Spacing with FullWidth {

  class DateWidget(caption: String, ussUI: UserSingleSelectWidget) extends HorizontalLayoutUI(caption, margin = false) {
  ...
  object info {
    ...
    val dCreated = new DateWidget("Created", ussCreator.widget)
    val dModified = new DateWidget("Modified", ussModifier.widget)
  }
  ...
  object publication {
    ...
    ussPublisher.widget.setCaption("Publisher")
  }
  ...
  private val pnlPublication = new Panel("Publication") with FullWidth {
    ...
    content.addComponents(publication.sltStatus, publication.sltVersion, lytDate, publication.ussPublisher.widget, publication.lytPhase)
  }
