...
class DocManager extends ImcmsServicesSupport {
  ...
  val docSelectionDlg = new OKDialog("doc.dlg_selection.caption".i) with CustomSizeDialog with Resizable with NonModal |>> {
    dlg =>
      dlg.mainComponent = customDocs.view
      dlg.setSize(500, 500)
  }

  val view = new DocManagerView(projection.view) |>> {
    w =>
    //    ui.miSelectionShow.setCommandHandler { _ =>
    //      Current.ui.addWindow(docSelectionDlg)
    //    }

      w.miShow.setCommandHandler {
        _ => projectionOps.showSelectedDoc()
      }
      w.miEdit.setCommandHandler {
        _ => projectionOps.editSelectedDoc()
      }

      w.miNewTextDoc.setCommandHandler {
        _ => projectionOps.mkDocOfType[TextDocumentDomainObject]
      }
      w.miNewFileDoc.setCommandHandler {
        _ => projectionOps.mkDocOfType[FileDocumentDomainObject]
      }
      w.miNewUrlDoc.setCommandHandler {
        _ => projectionOps.mkDocOfType[UrlDocumentDomainObject]
      }
      w.miNewHtmlDoc.setCommandHandler {
        _ => projectionOps.mkDocOfType[HtmlDocumentDomainObject]
      }
      w.miDelete.setCommandHandler {
        _ => projectionOps.deleteSelectedDocs()
      }

      w.miProfileEditName.setCommandHandler {
        _ =>
          whenSingleton(projection.docsView.selection) {
            docId =>
              ...
              new OkCancelDialog("Edit profile name") |>> {
                dlg =>
