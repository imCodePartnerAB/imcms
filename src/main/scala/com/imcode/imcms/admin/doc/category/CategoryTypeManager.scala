...
class CategoryTypeManager {
  ...
  val view = new CategoryTypeManagerView |>> {
    w =>
      w.miReload.setCommandHandler {
        _ => reload()
      }
      w.tblTypes.addValueChangeHandler {
        _ => handleSelection()
      }
    ...
    // fixme :  new CategoryTypeDomainObject params
      ...
      w.miDelete.setCommandHandler {
        _ =>
          whenSelected(w.tblTypes) {
            id =>
              new ConfirmationDialog("Delete selected category type?") |>> {
                dlg =>
  ...
  private def editAndSave(vo: CategoryTypeDomainObject) {
    ...
    new OkCancelDialog(dialogTitle) |>> {
      dlg =>
        dlg.mainComponent = new CategoryTypeEditorView |>> {
          w =>
                  ...
                  Current.ui.privileged(permission) {
                    Ex.allCatch.either(categoryMapper saveCategoryType voc) match {
                      case Left(ex) =>
                        ...
                        throw ex
                      case _ =>
                        (if (isNew) "New category type has been created" else "Category type has been updated") |> {
                          msg =>
  ...
  private def handleSelection() {
    (canManage && view.tblTypes.isSelected) |> {
      enabled =>
