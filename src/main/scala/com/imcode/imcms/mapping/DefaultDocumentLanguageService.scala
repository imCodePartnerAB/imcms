...
class DefaultDocumentLanguageService extends DocumentLanguageService with Log4jLoggerSupport {
  ...
  override def getByCode(code: String): DocumentLanguage = languageDao.findByCode(code) |> OrmToApi.toApi

  override def getAllLanguages: util.List[DocumentLanguage] = {
    languageDao.findAll.asScala.map(OrmToApi.toApi).asJava
  }
  ...
  override def getDefault: DocumentLanguage = systemDao.findByName("DefaultLanguageId") match {
    ...
    case property => property.getValue match {
      case NonNegInt(id) => languageDao.findOne(property.getValue.toInt).asOption match {
