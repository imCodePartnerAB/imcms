...
%><vel:velocity><html>
	<head>
		<title><fmt:message
				key="install/htdocs/sv/jsp/docadmin/document_information.jsp/document_information_title"/></title>
		...
		<script type="text/javascript"
				src="<%= cp %>/imcms/scripts/autocomplete/imcms_jquery.autocomplete.min.js"></script>
	...
	<table border="0" cellspacing="0" cellpadding="0">
		<tr>
			...
			<td><input type="button" value="<fmt:message key="global/help"/>"
					   ...
					   onClick="openHelpW('EditLink')"></td>
			<td>&nbsp;</td>
			<td><input type="button" id="editLinkAdvancedSettingsBtn"
					   value="<fmt:message key="global/AdvancedSettings"/>"
					   title="<fmt:message key="global/AdvancedSettings"/>" class="imcmsFormBtn"></td>
		</tr>
	</table>
	...
	if (DEBUG) { %>
HREF: <input type="text" id="<%= HREF.toString() %>" name="<%= HREF.toString() %>"
			 value="<%= StringEscapeUtils.escapeHtml4(href) %>" style="width:500px;"/><%
} else { %>
<input type="hidden" id="<%= HREF.toString() %>" name="<%= HREF.toString() %>"
	   value="<%= StringEscapeUtils.escapeHtml4(href) %>"/><%
	} %>
<table border="0" cellspacing="2" cellpadding="0" style="width:630px;">
	...
	<tr>
		...
		<td id="tdInput"><input type="text" id="<%= HREF.toString() %>dummy"
								value="<%= StringEscapeUtils.escapeHtml4(href) %>"
		...
		<td align="right"><input type="submit" class="imcmsFormBtnSmall toolTip" name="SEARCH" id="searchBtn"
								 ...
								 title="<fmt:message key="install/htdocs/imcms/html/link_editor.jsp/search/title"/>"/>
		</td>
	</tr>
	<tr>
		<td class="imcmsAdmText"><fmt:message key="edit/link/title"/></td>
		<td><input type="text" id="<%= TITLE.toString() %>" name="<%= TITLE.toString() %>"
				   value="<%= StringEscapeUtils.escapeHtml4(title) %>"
				   maxlength="300" size="60" style="width:100%;"/></td>
		<td>&nbsp;</td>
	</tr>
	<%
	...
	if ("_self".equalsIgnoreCase( target ) || "".equals( target )) {
		...
		target = null;
					...
					<td class="imcmsAdmText">&nbsp;<label for="target_self"><fmt:message
							key="install/htdocs/sv/jsp/docadmin/document_information.jsp/1015"/></label>&nbsp;</td>
					<td><input type="radio" name="<%= TARGET.toString() %>" id="target_blank" value="_blank"<%
	if ("_blank".equalsIgnoreCase( target ) ) {
		%> checked<%
		target = null;
	} %>></td>
					<td class="imcmsAdmText">&nbsp;<label for="target_blank"><fmt:message
							key="install/htdocs/sv/jsp/docadmin/document_information.jsp/1016"/></label>&nbsp;</td>
	...
	if ("_top".equalsIgnoreCase( target ) ) {
		...
		target = null;
					...
					<td class="imcmsAdmText">&nbsp;<label for="target_blank"><fmt:message
							key="install/htdocs/sv/jsp/docadmin/document_information.jsp/1017"/></label>&nbsp;</td>
					<td><input type="radio" name="<%= TARGET.toString() %>" id="target_other"
							   value="" <% if (null != target) { %> checked<% } %>></td>
					<td class="imcmsAdmText">&nbsp;<label for="target_other"><fmt:message
							key="install/htdocs/sv/jsp/docadmin/document_information.jsp/1018"/></label>&nbsp;</td>
					<td>
						<input type="text" name="<%= TARGET.toString() %>" size="9" maxlength="20" style="width:120px;"
							   value="<%
	if (null != target) {
...
<div id="advancedSettingsDiv" style="display:none;">
	<table border="0" cellspacing="2" cellpadding="0" style="width:630px;">
		<tr>
			<vel:velocity>
			<td colspan="3" style="padding-top:10px;">#gui_heading( '<fmt:message key="global/AdvancedSettings"/>' )
			</td>
		</tr>
		</vel:velocity>
		<tr>
			<td class="imcmsAdmText" nowrap="nowrap"><fmt:message key="edit/link/cssClass"/></td>
			<td><input type="text" id="<%= CLASS.toString() %>" name="<%= CLASS.toString() %>" size="20" maxlength="50"
					   style="width:100%;"
		...
		<tr>
			...
			<td><input type="text" id="<%= STYLE.toString() %>" name="<%= STYLE.toString() %>" size="20" maxlength="50"
					   style="width:100%;"
		...
		<tr>
			...
			<td><input type="text" id="<%= OTHER.toString() %>" name="<%= OTHER.toString() %>" size="20" maxlength="50"
					   style="width:100%;"
		...
		<tr>
			<td>&nbsp;</td>
			<td colspan="2" class="imcmsAdmDim"><fmt:message
					key="edit/link/otherParamsExample"/> <%= StringUtils.join(EditLink.OTHER_PARAMETERS, ", ") %>
			</td>
		</tr>
		<tr>
			<td style="padding:0;"><img src="<%= cp %>/imcms/swe/images/admin/1x1.gif" width="110" height="1" alt=""/>
			</td>
			<td style="padding:0;"><img src="<%= cp %>/imcms/swe/images/admin/1x1.gif" width="450" height="1" alt=""/>
			</td>
			<td style="padding:0;"><img src="<%= cp %>/imcms/swe/images/admin/1x1.gif" width="70" height="1" alt=""/>
			</td>
		</tr>
	</table>
</div>
...
<table border="0" cellspacing="2" cellpadding="0" style="width:630px;">
	...
	<tr>
		<td align="right"><input type="submit" class="imcmsFormBtn" name="<%= OkCancelPage.REQUEST_PARAMETER__OK %>"
								 value="<fmt:message key="global/OK"/>"/></td>
	</tr>
</table>
</form>
<vel:velocity>
...
<script type="text/javascript">
	...
	var defaultValueTextInternalLinks = "<%= StringEscapeUtils.escapeEcmaScript(isSwe ?
							...
							formatItem: function (array, i, max, value, term) {
								try {
									if (isStartsWithSearch) {
										return value;
										<%-- ('' == term) ? value : value.replace(new RegExp('^(' + term + ')(.*?)$', 'gi'), '<strong>$1</strong>$2') ;--%>
									} else {
										return value;
										<%-- ('' == term) ? value : value.replace(new RegExp('(' + term + ')', 'gi'), '<strong>$1</strong>') ;--%>
									}
								} catch (e) {
									return value;
								}
