 ...
 * Content loop DAO v2.
 */
public class ContentLoopDao2 extends HibernateTemplate {
	 ...
	 * Saves content loop.
	 *
	 * If content loop already exists deletes it fist along with contents.
	 */
	// TODO: refactor
	@Transactional
	public synchronized ContentLoop saveContentLoop(Integer documentId, ContentLoop contentLoop) {
		Long loopId = contentLoop.getId();
        //ContentIndexes contentIndexes = null;

        if (loopId == null) {
            //contentIndexes = new ContentIndexes();
            //contentIndexes.
        }
