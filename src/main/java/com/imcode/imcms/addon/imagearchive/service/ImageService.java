...
public class ImageService {
    ...
    public Images findById(final long imageId, final User user) {
        return (Images) template.execute(new HibernateCallback() {
    ...
    public Exif findExifByPK(long imageId, short type) {
        return (Exif) template.get(Exif.class, new ExifPK(imageId, type));
    }
    ...
    public Images createImage(File tempFile, ImageInfo imageInfo, String imageName, User user) {
        ...
        template.persist(originalExif);
        template.persist(changedExif);
    ...
    public void deleteImage(long imageId) {
        template.bulkUpdate("DELETE ImageCategories ic WHERE ic.imageId = ?", imageId);
        template.bulkUpdate("DELETE ImageKeywords ik WHERE ik.imageId = ?", imageId);
        template.bulkUpdate("DELETE Exif e WHERE e.imageId = ?", imageId);
        template.bulkUpdate("DELETE Images im WHERE im.id = ?", imageId);
        
        facade.getFileService().deleteImage(imageId);
    }
    
    public void updateFullData(final Images image, final List<Integer> categoryIds, 
    ...
    public void updateData(final Images image, final List<Integer> categoryIds, 
            final List<String> imageKeywords) {
        template.execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException, SQLException {
    ...
    private static Query buildSearchImagesQuery(Session session, SearchImageCommand command, boolean count, List<Integer> categoryIds, User user) {
    ...
    @Transactional(propagation=Propagation.SUPPORTS, readOnly=true)
    public int searchImagesCount(final SearchImageCommand command, final List<Integer> categoryIds, final User user) {
    ...
    @SuppressWarnings("unchecked")
    @Transactional(propagation=Propagation.SUPPORTS, readOnly=true)
    public List<Images> searchImages(final SearchImageCommand command, final Pagination pag, final List<Integer> categoryIds, final User user) {
        if (user.isDefaultUser() && categoryIds.isEmpty()) {
        ...
        return template.executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException, SQLException {
                List<Map<String, Object>> result = buildSearchImagesQuery(session, command, false, categoryIds, user)
    ...
    @SuppressWarnings("unchecked")
    @Transactional(propagation=Propagation.SUPPORTS, readOnly=true)
    public List<Categories> findImageCategories(final long imageId) {
        return template.executeFind(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException, SQLException {
    ...
    @Transactional(propagation=Propagation.SUPPORTS, readOnly=true)
    public boolean canUseImage(final User user, final long imageId) {
        if (user.isSuperAdmin()) {
            return true;
        }
    ...
    @SuppressWarnings("unchecked")
    @Transactional(propagation=Propagation.SUPPORTS, readOnly=true)
    public void setImageMetaIds(final Images image) {
        template.execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException, SQLException {
                List<Integer> metaIds = session.createSQLQuery(
    ...
    @SuppressWarnings("unchecked")
    @Transactional(propagation=Propagation.SUPPORTS, readOnly=true)
    public void setUsedInImcms(final List<Images> images) {
        template.execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException, SQLException {
                ...
                List<BigInteger> result = session.createSQLQuery(
                        "SELECT DISTINCT i.archive_image_id FROM images i WHERE i.archive_image_id IN (:imageIds)")
    ...
    @SuppressWarnings("unchecked")
    @Transactional(propagation=Propagation.SUPPORTS, readOnly=true)
    public void setImagesMetaIds(final List<Images> images) {
        template.execute(new HibernateCallback() {
            public Object doInHibernate(Session session) throws HibernateException, SQLException {
                Map<Long, Images> imageMap = new HashMap<Long, Images>(images.size());
                for (Images image : images) {
                ...
                List<Object[]> result = session.createSQLQuery(
                        "SELECT DISTINCT i.archive_image_id, i.meta_id FROM images i WHERE i.archive_image_id IN (:imageIds) " +
                        "ORDER BY i.archive_image_id, i.meta_id")
                        .setParameterList("imageIds", imageMap.keySet())
                        .list();
