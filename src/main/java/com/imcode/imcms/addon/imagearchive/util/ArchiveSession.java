...
public class ArchiveSession implements Serializable {
    ...
    private ArchiveSession(User belongsTo) {
    ...
    public static ArchiveSession getSession(HttpServletRequest request) {
        ...
        if (archiveSession == null || archiveSession.getBelongsTo().getId() != user.getId()) {
            archiveSession = new ArchiveSession(user);
