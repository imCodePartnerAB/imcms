...
class PropertyBasedTextContentFilter implements TextContentFilter {
	...
	private final Safelist plainTextWhitelist = Safelist.none().addTags("br", "p");
	private final Safelist basicHtmlTagsWhiteList = Safelist.basic().removeTags("span");
	...
	private void addCommonAttributesToWhiteLists(Safelist... whitelists) {
		for (Safelist whitelist : whitelists) {
			whitelist
	...
	@Override
	public String cleanText(String cleanMe, Safelist whitelist) {
		cleanMe = StringUtils.trimToEmpty(cleanMe);
		cleanMe = removeIllegalTags(cleanMe);
		Document.OutputSettings outputSettings = new Document.OutputSettings().prettyPrint(false);
		return StringEscapeUtils.unescapeXml(Jsoup.clean(cleanMe, "", whitelist, outputSettings));
	}
