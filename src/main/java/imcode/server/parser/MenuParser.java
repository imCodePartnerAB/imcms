...
public class MenuParser {
    ...
    public static String addMenuAdmin(int menuIndex, boolean menuMode, String content,
        ...
        request.setAttribute("menuIndex", menuIndex);
    ...
    public static String getPathToDocument(HttpServletRequest request, DocumentDomainObject document, String template, ParserParameters parserParameters) {
        ...
        if (StringUtils.isNotBlank(template)) {
            href += (-1 != href.indexOf('?')) ? '&' : '?';
            try {
                href += "template=" + URLEncoder.encode(template, "UTF-8");
    ...
    public void nodeMenu(Element menuNode, Writer result, MenuDomainObject currentMenu,
        ...
        if (menuNode.getChildElement("menuloop") == null) {
            ...
            for (Object o : menuNode.getChildren()) {
                Node menuNodeChild = (Node) o;
    ...
    private void nodeMenuLoop(Element menuLoopNode, Writer result, MenuDomainObject menu,
        ...
        List<Element> menuLoopNodeChildren = menuLoopNode.getChildren();
    ...
    private void loopOverMenuItemsAndMenuItemTemplateElementsAndAddToResult(MenuDomainObject menu,
                                                                            final List<Element> menuLoopNodeChildren,
                                                                            Writer result,
        ...
        while (menuItemsIterator.hasNext()) { // While there still are more menuitems from the db.
            for (Node menuLoopChild : menuLoopNodeChildren) {  // While there still are more imcms:menuloop-children
    ...
    private void nodeMenuItem(Element menuItemNode, Writer result, MenuItemDomainObject menuItem,
        ...
        for (Node menuItemChild : menuItemNode.getChildren()) { // For each node that is a child of this imcms:menuitem-element
            switch (menuItemChild.getNodeType()) { // Check the type of the child-node.
                case Node.ELEMENT_NODE: // An element-node
                    ...
                    if (!"menuitemhide".equals(menuItemChildElement.getName())
                            || menuItem != null)
                    { // if the child-element isn't a imcms:menuitemhide-element or there is a child...
    ...
    public String tag(Properties menuattributes, String menutemplate, TagParser tagParser) {
        int menuIndex;
        try {
    ...
    private Substitution getMenuItemSubstitution(MenuItemDomainObject menuItem,
        ...
        List<String> menuItemAHref = new ArrayList<>(4);
