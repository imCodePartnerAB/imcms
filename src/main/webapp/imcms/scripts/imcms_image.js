 ...
 * Updated by Serhii and 3emluk in 2016-2017
 */
(function (Imcms) {
    ...
    ImageCropper.prototype = {
        ...
        changeCropping: function (left, top, right, bottom) {
            ...
            var imageCroppingFrame = $(this._imageCroppingFrame),
                image = imageCroppingFrame.find("img"),
                grip = $(this._grip),
        ...
        collect: function () {
            var image = $(this._img),
                imageCroppingFrame = this._imageCroppingFrame,
                factor = image[0].naturalWidth / image.width(),
                x = parseInt((imageCroppingFrame.position().left + 1) * factor),
                y = parseInt((imageCroppingFrame.position().top + 1) * factor),
                width = parseInt(imageCroppingFrame.width() * factor),
                height = parseInt(imageCroppingFrame.height() * factor);
            return {
                cropX1: x,
                cropY1: y,
                cropX2: x + width,
        ...
        createImageCroppingFrame: function (image) {
            var imageCroppingFrame = $("<div>").addClass("image-cropping-frame").attr("id", "imageCroppingFrame"),
                width = (this.destinationWidth === null) ? 100 : this.destinationWidth,
                height = (this.destinationHeight === null) ? 100 : this.destinationHeight,
                factor;
    ...
    ImageInfoAdapter.prototype = {
        ...
        updateCropping: function (croppingOptions) {
            ...
            var zoomFactor = 1,
                noDivWidth = isNaN(this._divWidth),
                noDivHeight = isNaN(this._divHeight);
            ...
            function getDivProportion(divProp, hasNotAnotherProp, cropProp) {
                if (isNaN(divProp)) {
                    ...
                    return (cropProp > divProp) ? divProp : cropProp;
                }
            }
        ...
        _validate: function (inputName) {
            var $infoRef = $(this._infoRef.getHTMLElement()),
                element = $infoRef.find("input[name=" + inputName + "]");
        ...
        _deformationCheck: function () {
            var $infoRef = $(this._infoRef.getHTMLElement()),
                divHeightEl = $infoRef.find("input[name=divHeight]");
    ...
    var ImageViewAdapter = function (options) {
        this.init(options);
    };
    ImageViewAdapter.prototype = {
        _loader: {},
        _options: {},
        _parent: {},
        _imageView: {},
    ...
    Editor.prototype = {
        ...
        init: function () {
            ...
            if (data.loop && data.entry) {
                ...
                this._loader.getById(this, this.initSource.bind(this));
            }
        },
        ...
        buildCropper: function (data) {
            if (this._imageCropper instanceof ImageCropper) {
        ...
        buildImageView: function (data) {
            ...
            this._imageViewAdapter = new ImageViewAdapter({
        ...
        buildInfoView: function (data) {
            ...
            this._infoViewAdapter = new ImageInfoAdapter({
        ...
        _onSaveReloadTask: function (showProcessWindow) {
            ...
            return {
                ...
                callback: function () {
                    new Editor($element[0], Imcms.Editors.Image);
                }
            }
        },
    ...
    var ImageInTextEditorWindow = function (textEditor) {
        this._textEditor = textEditor;
        this.init();
    };
    ImageInTextEditorWindow.prototype = {
        _realElement: {},
        _element: {},
        _id: {},
        _meta: {},
        ...
        mixinFromImageEditor: function (functionName) {
            this[functionName] = Editor.prototype[functionName].bind(this);
        },
        init: function () {
            this._loader = Imcms.Editors.Image;

            var editorKeys = Object.keys(Editor.prototype)
                .filter(function (key) {
                    return (typeof Editor.prototype[key] === 'function')
                });
            Object.keys(ImageInTextEditorWindow.prototype).forEach(function (key) {
        ...
        getCurrentImageWithCallback: function (callback) {
            this._loader.getById(this, callback);
        },
        ...
        save: function () {
            ...
            if (collectedData.name) {
                this._loader.save(this, collectedData, this._onSaveChangesCallback.bind(this));
            }
        ...
        _onRemoveImage: function () {
            ...
            this._loader.remove(this, this.close.bind(this));
        }
    };

    Imcms.Image = {};
    Imcms.Image.ImageInTextEditor = function (textEditor) {
        this._window = new ImageInTextEditorWindow(textEditor);
    };
    ...
    function generateApiUrl(request) {
        ...
        return Imcms.Linker.get("content.image", request.object) + (
                (searchURL && searchURL.charAt(0) === "?")
                    ? "&" + searchURL.slice(1)
                    : ""
            );
    }
    ...
    var api = {
        read: function (request, response) {
            ...
            Imcms.Logger.log("Image.API::read :", Imcms.REST.get.bindArgs(url, {}, response));
        },
        update: function (request, response) {
            var url = generateApiUrl(request);
            Imcms.Logger.log("Image.API::update :", Imcms.REST.post.bindArgs(url, request, response));
        },
        remove: function (request, response) {
            var url = generateApiUrl(request);
            Imcms.Logger.log("Image.API::remove :", Imcms.REST["delete"].bindArgs(url, request, response));
        }
    };
    ...
    Imcms.Image.Loader.prototype = {
        ...
        initEditor: function (position, element) {
            this._editorsList[position] = new Editor(element, this);
        },
        getById: function (docData, callback) {
            api.read({object: resolveImageStringObject(docData)},
                Imcms.Logger.log.bind(this, "Image::getById : ", callback));
        },
        getByLoopItemRef: function (docData, callback) {
            api.read(
                {object: resolveLoopImageStringObject(docData)},
                Imcms.Logger.log.bind(this, "Image::getByLoopItemRef : ", callback)
            );
        },
        getByPath: function (path, callback) {
            api.read({object: path}, callback);
        },
        save: function (docData, imageData, callback) {
            api.update({
                    sharedMode: docData._infoViewAdapter.isSharedMode(),
                    imageDomainObject: JSON.stringify(imageData),
                    object: resolveImageStringObject(docData)
                },
                Imcms.Logger.log.bind(this, "Image::save : ", callback)
            );
        },
        saveLoopItem: function (docData, imageData, callback) {
            api.update({
                sharedMode: docData._infoViewAdapter.isSharedMode(),
                imageDomainObject: JSON.stringify(imageData),
                object: resolveLoopImageStringObject(docData)
        ...
        remove: function (docData, callback) {
            api.remove({object: resolveImageStringObject(docData)},
                Imcms.Logger.log.bind(this, "Image::remove : ", callback));
        },
        removeLoopItem: function (docData, callback) {
            api.remove(
                {object: resolveLoopImageStringObject(docData)},
