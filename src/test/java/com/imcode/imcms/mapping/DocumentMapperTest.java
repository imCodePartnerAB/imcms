...
public class DocumentMapperTest {
    ...
    static DocumentMapper docMapper;

    static UserDomainObject admin;

    static UserDomainObject user;

    static I18nSupport i18nSupport;
...
//    }
    ...
    @Test(expected = NoPermissionToAddDocumentToMenuException.class)
...
//    }
    ...
    public void changeDocumentDefaultVersion() throws Exception {
        ...
        assertEquals("Default version of a new document is 0.", doc.getVersion().getNo(), new Integer(0));

        DocumentVersion version = docMapper.makeDocumentVersion(docId, admin);

        assertEquals("New doc version no is 1.", version.getNo(), new Integer(1));

        docMapper.changeDocumentDefaultVersion(docId, 1, admin);

        doc = docMapper.getDefaultDocument(docId, i18nSupport.getDefaultLanguage());

        assertEquals("Default version of a document is 1.", doc.getVersion().getNo(), new Integer(1));
    }
