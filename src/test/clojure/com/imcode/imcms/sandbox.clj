...
(defn filter-releases [releases major-no & major-nos]
...
(defn test-releases []
  (create-releases *TEST-TAG-NAMES*))


(defonce *SVN-URL-IMCMS-TAGS* "https://repo.imcode.com/imcode/imcms/tags")


(defn release-name [{:keys [major-no minor-no revision-no build-name build-no]}]
...
(defn get-releases [username password]
  (let [repo (login *SVN-URL-IMCMS-TAGS* username password)
        releases-entries (filter directory? (dir repo ""))
        releases-names (map #(.getName %) releases-entries)]
    (create-releases releases-names)))
...
(defn final-release? [release]
  (nil? (:build-name release)))

(defn pre-release? [release]
  (not (final-release? release)))

(defn last-final-release [releases]
  (last (filter final-release? (sort-releases releases))))

(defn last-release [releases]
  (last (sort-releases releases)))


(defn next-final-release [releases major-no]
  (if-let [release (last-release (filter-releases releases major-no))]
...
(defn next-pre-release
  "Generates next pre-release based on major-no."
  [releases major-no]
  (if-let [release (last-release (filter-releases releases major-no))]
