...
(def release-name-re #"\d+\.\d+\.\d(-(alpha|beta)\d+)?")

(defn release-struct
  "Creates release struct map from a release name."
  [#^String release-name]
  ;; TODO: re...
  {:release-name nil :major-no nil :minor-no nil :revision-no nil :build-name nil :build-no nil})

(defn release-name?
  ""
  [name]
  (re-matches release-name-re name))
