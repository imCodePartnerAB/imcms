...
<%
    SearchDocuments.SearchDocumentsPage searchDocumentsPage = (SearchDocuments.SearchDocumentsPage)request.getAttribute( SearchDocuments.SearchDocumentsPage.REQUEST_ATTRIBUTE__PAGE ) ;
...
<table width="550" border="0" cellspacing="0">
    ...
    <tr>
        <td colspan="2" class="imcmsAdmText"><font face="Verdana, Arial, Helvetica, sans-serif">
            <input type="text" name="<%= SearchDocuments.REQUEST_PARAMETER__QUERY %>" value="<%= StringEscapeUtils.escapeHtml(StringUtils.defaultString(searchDocumentsPage.getQuery())) %>" size="65" style="width: 100%"></font></td>
    </tr>
    <tr>
        <td colspan="2">&nbsp;</td>
    </tr>
        ...
        <tr>
            ...
            <tr>
                ...
                <td class="imcmsAdmText">
                    <select name="<%= SearchDocuments.REQUEST_PARAMETER__DOCUMENTS_PER_PAGE %>" size="1">
                        <%
                ...
                <td class="imcmsAdmText"><font face="Verdana, Arial, Helvetica, sans-serif">&nbsp;&nbsp;<? templates/sv/search/search_documents.html/5 ?>&nbsp;</font></td>
                <td class="imcmsAdmText"><font face="Verdana, Arial, Helvetica, sans-serif">
                <select name="<%= SearchDocuments.REQUEST_PARAMETER__SECTION_ID %>">
    ...
    <tr>
        <td colspan="2" align="right">
            <input type="submit" name="<%= SearchDocuments.REQUEST_PARAMETER__SEARCH_BUTTON %>" class="imcmsFormBtn" value="<? templates/sv/search/search_documents.html/1 ?>">
        </td>
    <tr>
</table>
</form>
            ...
            <td colspan="2">
                ...
                <% } else { %>
                    ...
                    for ( int i = firstDocumentIndex ; i < documentsFound.length
                        ...
                        <tr valign="top" <% if (0 == i % 2) { %> bgcolor="#FFFFFF"<% } %>>
                            <td>
                                    <%
                            ...
                            <td align="right">&nbsp;<% if (null != searchDocumentsPage.getSelectDocumentUrlPrefix()) { %><a href="<%= searchDocumentsPage.getSelectDocumentUrlPrefix() + document.getId() %>"><? imcms/lang/jsp/search_documents.jsp/select_document ?></a><% } %></td>
                        </tr>
                    <% }
                } %>
                </table>
            ...
            <td colspan="2">
                <%
                    if (firstDocumentIndex > 0) {
                        ...
                        <a href="SearchDocuments?<%= SearchDocuments.REQUEST_PARAMETER__FIRST_DOCUMENT_INDEX %>=<%= firstDocumentIndexOnPreviousPage + "&"+ Utility.getQueryStringExcludingParameter(request, SearchDocuments.REQUEST_PARAMETER__FIRST_DOCUMENT_INDEX) %>">
                    ...
                    for (int i = 0; (i * documentsPerPage) < documentsFound.length && documentsFound.length > documentsPerPage; i++) { %>
                        <a href="SearchDocuments?<%= SearchDocuments.REQUEST_PARAMETER__FIRST_DOCUMENT_INDEX %>=<%= (i * documentsPerPage) + "&"+ Utility.getQueryStringExcludingParameter(request, SearchDocuments.REQUEST_PARAMETER__FIRST_DOCUMENT_INDEX) %>"><%= i + 1 %></a>&nbsp;<%
                    }
                    int firstDocumentIndexOnNextPage = ( firstDocumentIndex + documentsPerPage );
                    if (documentsFound.length > firstDocumentIndexOnNextPage) { %>
                        <a href="SearchDocuments?<%= SearchDocuments.REQUEST_PARAMETER__FIRST_DOCUMENT_INDEX %>=<%= firstDocumentIndexOnNextPage + "&"+ Utility.getQueryStringExcludingParameter(request, SearchDocuments.REQUEST_PARAMETER__FIRST_DOCUMENT_INDEX) %>">
