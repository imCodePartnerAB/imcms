...
/* *******************************************************************************************
 *         imCode specific functions                                                         *
 ******************************************************************************************* */

var arrTabBeforeElements      = [
	"<td", "<li", "<dt", "<dd"
] ;
var arrOneBreakBeforeElements = [
	"<table", "</table>", "<tbody", "</tbody>", "<tr", "</tr>", "<td", "<blockquote", "</blockquote>", "<dl", "</dl>", "<dt", "<dd", "<ol", "</ol>", "<ul", "</ul>", "<li"
] ;
var arrTwoBreakBeforeElements = [] ;

var arrOneBreakAfterElements  = [
	"<br>", "</dl>", "</ol>", "</ul>"
] ;
var arrTwoBreakAfterElements  = [
	"</p>", "</blockquote>"
] ;
...
HTMLArea.Config = function () {
	...
	this.metaId      = 1001 ;        // TU - set onLoad in config
	this.servletPath = "/servlet/" ; // TU - set onLoad in config
	this.contextPath = "/" ;         // TU - set onLoad in config
...
HTMLArea.loadStyle("htmlarea.css.jsp");
	...
	if (!text) {
		...
		if (this.config.statusBar && !noStatus) {
			...
			for (var i = ancestors.length; --i >= 0;) {
				...
				var txt = (el.tagName.toLowerCase() == "body") ? "content" : el.tagName.toLowerCase();
...
HTMLArea.prototype._createLink = function(link) {
	...
	if (typeof link == "undefined") {
		...
		if (link && !/^a$/i.test(link.tagName)) {
			link = null;
			arrAncestors   = this.getAllAncestors() ;
			sAncestors = "" ;
			if (arrAncestors != null) {
				for (var i = 0; i < arrAncestors.length; i++) {
					sAncestors += arrAncestors[i].tagName + "\n" ;
					if (/^a$/i.test(arrAncestors[i].tagName)) {
						link = arrAncestors[i] ;
						break ;
					}
				}
	...
	var isImage = /<img/gi.test(editor.getSelectedHTML()) ;
	var selText = editor.getSelectedHTML().replace(/(<[^>]+?>)|(&nbsp;)/gi,"") ;
	//alert(editor.getSelectedHTML() + "\n\n" + selText) ;
	if (!link && !/\w+/.test(selText) && !isImage) {
		alert(HTMLArea.I18N.msg["Select_text_first"]) ;
		return false ;
	}
	if (link) outparam = {
...
HTMLArea.prototype._insertImage = function(image) {
	...
	if (image) {
		outparam = {
			imageref     : HTMLArea.is_ie ? editor.stripBaseURL(image.src) : image.getAttribute("src"),
			image_name   : image.name,
			image_width  : image.width,
			image_height : image.height,
			image_border : image.border,
			v_space      : image.vspace,
			h_space      : image.hspace,
			image_align  : image.align,
			alt_text     : image.alt
		};
	}
	this._popupDialog(editor.config.servletPath + "ChangeImage?meta_id=" + editor.config.metaId + "&img=76348&editor_image=true", function(param) {
		...
		if (!img) {
			...
			var range = editor._createRange(sel);
			if (HTMLArea.is_ie) {
				editor._doc.execCommand("insertimage", false, param.imageref);
		...
		for (field in param) {
			...
			switch (field) {
			    //case "imageref"     : img.src    = value ; break ;
			    case "image_name"   : img.name   = value ; break ;
			    case "image_width"  : img.width  = parseInt(value || "0") ; break ;
			    case "image_height" : img.height = parseInt(value || "0") ; break ;
			    case "image_border" : img.border = parseInt(value || "0") ; break ;
			    case "v_space"      : img.vspace = parseInt(value || "0") ; break ;
			    case "h_space"      : img.hspace = parseInt(value || "0") ; break ;
			    case "image_align"  : img.align  = value ; break ;
			    case "alt_text" :
						img.alt   = value ;
						img.title = value ;
						break ;
...
HTMLArea.checkSupportedBrowser = function() {
	...
	} else if (HTMLArea.is_ie && !isIE55) {
			alert("You need at least Internet Explorer 5.5.\n" +
			      "Sorry, your Internet Explorer is not supported.");
			return false;
	}
...
HTMLArea.prototype.stripBaseURL = function(string) {
	...
	var basere1 = new RegExp(baseurl);
	var basere2 = new RegExp(this.config.servletPath.substring(1,this.config.servletPath.length - 1)) ;
	baseurl = baseurl.replace(basere2, "");
	...
	basere1 = new RegExp(baseurl);
	string = string.replace(basere1, "");
	// strip host-part of URL which is added by MSIE to links relative to server root
	baseurl = baseurl.replace(/^(https?:\/\/[^\/]+)(.*)$/, "$1");
	basere1 = new RegExp(baseurl);
	return string.replace(basere1, "");
};
